...
test_cache_hit() {
  ...
  headers_hit=$(get_all_headers "$url")

  local ok=0
  local nc cc st lm

  nc=$(extract_header "nmd-cache"     "$headers_hit")
  cc=$(extract_header "cache-control" "$headers_hit")
  st=$(extract_header "server-timing" "$headers_hit")
  lm=$(extract_header "last-modified" "$headers_hit")
